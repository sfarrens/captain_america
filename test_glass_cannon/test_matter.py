...
from glass_cannon.matter import run_ln_fields, run_discretized_cls, run_solve_gauss_spectra, run_generate
...
def test_run_solve_gauss_spectra():
    ...
    expected_gls = glass.solve_gaussian_spectra(fields, cls)

    gls = run_solve_gauss_spectra(fields, cls)

    assert all(np.allclose(a, b, rtol=1e-6, atol=1e-8) for a, b in zip(expected_gls, gls))
...
def test_run_generate():
    ...
    expected_matter = glass.generate(fields, gls, nside, ncorr=3, rng=rng)

    rng = np.random.default_rng(seed=42)
    matter = run_generate(fields, gls, nside, rng=rng)

    assert all(np.allclose(a, b, rtol=1e-6, atol=1e-8) for a, b in zip(expected_matter, matter))
